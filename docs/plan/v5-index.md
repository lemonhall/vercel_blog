...
- 2026-06-11 后补丁补齐 tags 多选筛选：`/recipes?tags=<slug1>,<slug2>` 使用 AND 交集，只展示同时包含所有已选 tags 的已发布食谱；点击已选 tag 会取消该 tag；“全部取消”回到无 tag 筛选；旧 `/recipes/tags/<tag>` 兼容跳转到 `/recipes?tags=<tag>`。
